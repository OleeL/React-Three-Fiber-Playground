...
DRACOLoader.prototype = Object.assign( Object.create( Loader.prototype ), {
	...
	decodeGeometry: function ( buffer, taskConfig ) {
		...
		for ( let attribute in taskConfig.attributeTypes ) {
			...
			let type = taskConfig.attributeTypes[ attribute ];
		...
		if ( DRACOLoader.taskCache.has( buffer ) ) {
			...
			//@ts-ignore
			let cachedTask = DRACOLoader.taskCache.get( buffer );
		...
		let geometryPending = this._getWorker( taskID, taskCost )
			.then( ( _worker ) => {
	...
	_createGeometry: function ( geometryData ) {
		...
		for ( let i = 0; i < geometryData.attributes.length; i ++ ) {
			...
			let attribute = geometryData.attributes[ i ];
	...
	_initDecoder: function () {
		...
		let useJS = typeof WebAssembly !== 'object' || this.decoderConfig.type === 'js';
		let librariesPending = [];
		...
		this.decoderPending = Promise.all( librariesPending )
			.then( ( libraries ) => {
				...
				//@ts-ignore
				let fn = DRACOLoader.DRACOWorker.toString();
...
DRACOLoader.DRACOWorker = function () {
	...
	onmessage = function ( e ) {
		...
		switch ( message.type ) {
			...
			case 'decode':
				...
				let taskConfig = message.taskConfig;
				decoderPending.then( ( module ) => {
					...
					try {
						...
						let geometry = decodeGeometry( draco, decoder, decoderBuffer, taskConfig );
	...
	function decodeGeometry( draco, decoder, decoderBuffer, taskConfig ) {
		...
		if ( geometryType === draco.TRIANGULAR_MESH ) {
			...
			let index = new Uint32Array( numIndices );
			let indexArray = new draco.DracoInt32Array();
